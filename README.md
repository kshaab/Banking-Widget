# Banking Widget

## Содержание 📚

- [О проекте](#о-проекте)
- [Статус проекта](#статус-проекта)
- [Стек технологий](#стек-технологий)
- [Структура проекта](#структура-проекта)
- [Использование](#использование)
- [Основной функционал](#основной-функционал)
- [Зависимости](#зависимости)
- [Тестирование](#тестирование)
- [Автор](#автор)
- [Лицензия](#лицензия)

## О проекте
**Banking Widget** — учебный проект на Python для анализа транзакций, которые находятся в Excel-файле. Проект демонстрирует работу с данными, маскирование конфиденциальной информации, фильтрацию, сортировку и анализ транзакций.

**Основные возможности:**
- Маскирование номеров карт и счетов
- Фильтрация и сортировка операций
- Работа с JSON / CSV / Excel
- Генераторы и декораторы
- Конвертация валют через API
- Тестирование с помощью pytest

## Статус проекта 
✅ Завершен 

🎓 Учебный проект

## Стек технологий
- Python 3.10+
- Poetry
- Pytest
- JSON / CSV / Excel
- REST API

## Структура проекта
src/
  
     ├─ masks/
     ├─ widget/     
     ├─ processing/     
     ├─ generators/     
     ├─ decorators/    
     ├─ utils/     
     ├─ external_api/    
     ├─ read_transactions/   
     └─ actions_with_bank_operations/
 
tests/

main.py

## Использование
1. Клонирование репозитория: 
```bash
git clone https://github.com/kshaab/Banking-Widget
cd widget_project
```
2. Установка зависимостей и активация виртуального окружения:
```bash
poetry install
poetry shell
```
3. Запуск файла:
```bash 
python src/main.py
```

## Основной функционал

### 🔸 Masks

`get_mask_card_number(card_number)`

Маскирует номер  банковской карты.

```python
>>> get_mask_card_number("7000792289606361")
"7000 79** **** 6361"
```

 `get_mask_account(account)` 
 
 Маскирует номер счета.
 
```python
>>> get_mask_account("73654108430135874305")
"**4305"
```

### 🔸 Widget

`mask_account_card(info)` 

Маскирует номер карты, сохраняя ее название.

```python
>>> mask_account_card("Visa Platinum 7000792289606361")
"Visa Platinum 7000 79** **** 6361"
```

`get_date(date_str)` 

Преобразует формат даты.

```python
>>> get_date("2024-03-11T02:26:18.671407")
"11.03.2024"
```

### 🔸 Processing

`filter_by_state(data, state)` 

Фильтрует список словарей по ключу.

```python
filter_by_state(data, state="CANCELED")
```

`sort_by_date(info, reverse)` 
Сортирует список словарей по дате в порядке возрастания/убывания. 

```python
sort_by_date(data, reverse=True)
```

### 🔸 Generators

`filter_by_currency(transactions, currency_code)`

Возвращает итератор транзакций по валюте.

```python
filter_by_currency(transactions, "USD")
```

`transaction_descriptions(transactions)`

Поочерёдно возвращает описания операций.

```python
transaction_descriptions(transactions) 
```

`card_number_generator(start, stop)`

Генерирует номера карт в заданном диапазоне.

```python
for card_number in card_number_generator(1, 5)
```

### 🔸 Decorators 

`log(filename = None)`
Декоратор для логирования результатов функций.

```python
@log(filename="mylog.txt")
def my_function():
    return 3

"my_function ok, result: 3."
```

### 🔸 Utils 

`get_transactions(file_path)`

Загружает транзакции из JSON-файла.

### 🔸 External API 

`convert_to_rub`

Конвертирует валюту в рубли через API.

`get_rub_amount(transaction)`

Возвращает суммы транзакций в рублях. 

### 🔸 Read Transactions 

`read_transactions_from_csv(csv_data)`

Считывает финансовые операции из CSV-файла и возвращает список словарей. 

`read_transactions_from_excel(excel_dir)`

Считывает финансовые операции из EXCEL-файла и возвращает список словарей.

### 🔸 Actions with Bank Operations

`process_bank_search(bank_operations, search)`

Ищет операции по ключевому слову.

`process_bank_operations(bank_operations, unique_categories)`

Формирует статистику по категориям.

### 🔸 Main 

` print_transactions(result)`

Выводит отформатированную информацию.

`main()`
Запускает основной сценарий работы программы.


## Зависимости

Управление зависимостями осуществляется через Poetry.
Файл конфигурации: `pyproject.toml` 


## Тестирование

В проекте используется фреймворк pytest.

Запуск тестов:
```bash
pytest tests/ -v
```

## Автор
[Ксения](https://github.com/kshaab)

## Лицензия
Проект распространяется в учебных целях.
